...
BASE_INTERVAL = 1.5 #second between task gen per 1 worker
MIN_INTERVAL = 0.5 #never gen faster than this
MAX_QUEUE_PER_WORKER = 8 #stop gen if queue has more than MAX_QUEUE_PER_WORKER tasks per worker
...
def matrix_difficulty(size : int, iterations : int) -> float:
    difficulty_score = (size / 100) + (iterations * 0.5)
...
#-----------generate tasks--------------------
def generate_primes_task() -> 'SimulationTask':
    ...
    logger.debug(f"Generated primes task: range = {end_range}, task difficulty = {difficulty}")
    return task 

def generate_monte_task() -> 'SimulationTask':
    ...
    logger.debug(f"Generated monte task: arrows = {num_samples}, task difficulty = {difficulty}")
    return task

def generate_matrix_task() -> 'SimulationTask':
...
def generate_render_task() -> 'RenderTask':
...
async def task_pool_loop(lb : LoadBalancer) -> None:
    """
    Generate random tasks and sumbit them to LB.
    The gen rate is based on the number of connected workers:
        interval = max((BASE_INTERVAL / worker_num) , MIN_INTERVAL)
    """
    genartion_options = [generate_primes_task, generate_monte_task, generate_matrix_task, generate_render_task]
    ...
    while True:
        ...
        logger.info(
            f"submitted: [{task.__class__.__name__}] | id ={task.task_id[:8]} | difficulty = {task.task_difficulty} | priority = {priority.name} | workers ={num_workers} | queue ={q_size}"
        )
